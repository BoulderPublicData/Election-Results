...
# 2. Run the full pipeline — fetch, clean, audit
uv run python -m scripts.pipeline

# 3. Or just clean what's already on disk (faster, no network)
uv run python -m scripts.pipeline --skip-fetch --no-pdf

# 4. Skip slow PDF parsing if you only want machine-readable years
uv run python -m scripts.pipeline --skip-fetch --no-pdf
```
...
├── .github/workflows/
│   └── annual-sov-refresh.yml  # cron: every Jan 6 14:00 UTC
...
A [GitHub Action](.github/workflows/annual-sov-refresh.yml) runs every **January 6** (federal Electoral College certification day) and opens a PR with any newly-published files from the prior year. Run it manually any time via the **Actions** tab → **Annual SOV refresh** → *Run workflow*.
