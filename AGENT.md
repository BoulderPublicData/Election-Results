...
8. **Repo / org setting prerequisite for the Action:** The Action opens a PR via `peter-evans/create-pull-request`. For that to work, the repo (and the org, if it enforces) must have **Settings → Actions → General → "Allow GitHub Actions to create and approve pull requests"** enabled. Without it, the first scheduled run will fail at the `Open PR` step with `"GitHub Actions is not permitted to create or approve pull requests"`. The workflow now has a fallback step that opens an issue containing the same body in that case, but enabling the toggle is the proper fix.
